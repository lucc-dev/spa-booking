...
public class BookingTools {
    ...
    public String checkAvailability(
        ...
        try {
            ...
            // 加入過去時間檢查
            validateNotPastDate(date, time);

            int remaining = bookingService.getRemainingSlots(date, time);
    ...
    // 新增私有方法：檢查預約時間是否早於當前時間
    private void validateNotPastDate(LocalDate date, LocalTime time) {
        if (date == null || time == null) {
            throw new IllegalArgumentException("日期與時間不可為空。");
        }
        LocalDateTime bookingDateTime = LocalDateTime.of(date, time);
        if (bookingDateTime.isBefore(LocalDateTime.now())) {
            throw new IllegalArgumentException("無法預約已經過去的時間，請選擇未來的時段。");
        }
    }
}