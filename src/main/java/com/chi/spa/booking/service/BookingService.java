...
public class BookingService {
    ...
    public Booking createBooking(BookingRequest request) {
        ...
        validateNotPastDate(request.getBookingDate(), request.getBookingTime());
    ...
    public Booking updateBooking(Long id, Long newServiceItemId, LocalDate newDate, LocalTime newTime) {
        ...
        if (newDate != null || newTime != null) {
            ...
            validateNotPastDate(targetDate, targetTime);
    ...
    private void validateNotPastDate(LocalDate date, LocalTime time) {
        if (date == null || time == null) {
            throw new BusinessException("預約失敗：日期與時間不可為空。");
        }

        LocalDateTime bookingDateTime = LocalDateTime.of(date, time);
        if (bookingDateTime.isBefore(LocalDateTime.now())) {
            throw new BusinessException("預約失敗：無法預約已經過去的時間，請選擇未來的時段。");
        }
    }
